...
<div class="space-y-6">
    <div class="p-8 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-slate-100 gap-4">
            <div>
                <h2 class="text-base font-extrabold text-slate-900">Daftar Armada Mobil Dinas</h2>
                <p class="text-xs text-slate-500">Atur nomor polisi, kapasitas maksimal tempat duduk (kursi), dan status operasional kendaraan.</p>
            </div>
            <button type="button" onclick="openAddVehicleModal()" class="px-4 py-2 text-xs font-bold rounded-xl kalbe-gradient text-white shadow-md shadow-kalbe-500/20 hover:opacity-95 flex items-center space-x-2">
...
<div id="vehicleModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
    <div class="bg-white rounded-3xl p-8 max-w-lg w-full shadow-2xl border border-slate-200 space-y-6">
        <div class="flex items-center justify-between pb-4 border-b border-slate-100">
            <h3 class="font-extrabold text-base text-slate-900" id="modalTitle">Tambah Kendaraan Dinas</h3>
            <button type="button" onclick="closeVehicleModal()" class="text-slate-400 hover:text-slate-600"><i class="fa-solid fa-xmark text-lg"></i></button>
        </div>
        ...
        <form id="vehicleForm" action="{{ route('admin.vehicles.store') }}" method="POST" class="space-y-4">
            ...
            <div class="flex items-center justify-end space-x-3 pt-4 border-t border-slate-100">
                <button type="button" onclick="closeVehicleModal()" class="px-4 py-2 text-xs font-bold rounded-xl bg-slate-100 text-slate-700 hover:bg-slate-200">Batal</button>
                <button type="submit" class="px-6 py-2 text-xs font-bold rounded-xl kalbe-gradient text-white shadow-md shadow-kalbe-500/20 hover:opacity-95">Simpan Data</button>
            </div>
        </form>
    </div>
</div>
