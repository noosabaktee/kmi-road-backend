...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="p-8 rounded-3xl bg-white border border-slate-200 shadow-sm">
        <div class="pb-6 border-b border-slate-100 mb-6">
            <h2 class="text-xl font-extrabold text-slate-900">Formulir Pembuatan Jadwal Dinas Langsung</h2>
            <p class="text-xs text-slate-500 mt-1">Admin HC dapat langsung menentukan mobil operasional, menugaskan driver, dan memasukkan daftar nama karyawan penumpang.</p>
        </div>

        <form action="{{ route('admin.trips.store') }}" method="POST" id="directTripForm" class="space-y-8">
            @csrf

            <!-- Step 1: Penugasan Kendaraan & Driver -->
            <div>
                <h3 class="text-sm font-bold text-slate-800 uppercase tracking-wider mb-4 pb-2 border-b border-slate-100 flex items-center space-x-2">
                    <span class="w-6 h-6 rounded-md bg-kalbe-100 text-kalbe-700 flex items-center justify-center text-xs">1</span>
                    <span>Pilihan Kendaraan & Driver</span>
                </h3>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        <label for="intVehicle_ID" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Pilih Mobil Operasional <span class="text-red-500">*</span></label>
                        <select name="intVehicle_ID" id="intVehicle_ID" required class="w-full p-3 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-kalbe-500">
                    ...
                    <div>
                        <label for="intDriver_ID" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Tugaskan Driver (Supir) <span class="text-red-500">*</span></label>
                        <select name="intDriver_ID" id="intDriver_ID" required class="w-full p-3 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-kalbe-500">
                    ...
                    <div>
                        <label for="dtmTripDate" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Tanggal Dinas <span class="text-red-500">*</span></label>
                        <input type="date" name="dtmTripDate" id="dtmTripDate" value="{{ old('dtmTripDate', date('Y-m-d')) }}" required
                            class="w-full p-3 rounded-xl border border-slate-200 text-sm font-semibold focus:ring-2 focus:ring-kalbe-500">
                    </div>

                    <div>
                        <label for="dtmDepartureTime" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Waktu / Jam Berangkat (Opsional)</label>
                        <input type="datetime-local" name="dtmDepartureTime" id="dtmDepartureTime" value="{{ old('dtmDepartureTime') }}"
                            class="w-full p-3 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-kalbe-500">
                    </div>
                </div>
            </div>
            ...
            <div>
                <h3 class="text-sm font-bold text-slate-800 uppercase tracking-wider mb-4 pb-2 border-b border-slate-100 flex items-center space-x-2">
                    <span class="w-6 h-6 rounded-md bg-kalbe-100 text-kalbe-700 flex items-center justify-center text-xs">2</span>
                    <span>Tujuan & Keperluan Dinas</span>
                </h3>

                <div class="space-y-4">
                    <div>
                        <label for="txtDestination" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Tujuan Lokasi <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        <label for="txtPurpose" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Keperluan Dinas <span class="text-red-500">*</span></label>
                        <textarea name="txtPurpose" id="txtPurpose" rows="2" required placeholder="Jelaskan keperluan perjalanan dinas..."
                            class="w-full p-3 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-kalbe-500">{{ old('txtPurpose') }}</textarea>
                    </div>

                    <div>
                        <label for="txtNotes" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Catatan Khusus untuk Driver</label>
            ...
            <div>
                <div class="flex items-center justify-between pb-2 border-b border-slate-100 mb-4">
                    <h3 class="text-sm font-bold text-slate-800 uppercase tracking-wider flex items-center space-x-2">
                        <span class="w-6 h-6 rounded-md bg-kalbe-100 text-kalbe-700 flex items-center justify-center text-xs">3</span>
                        <span>Daftar Karyawan yang Ditugaskan (Penumpang)</span>
                    </h3>
                    <button type="button" onclick="addPassengerRow()" class="px-3 py-1.5 rounded-lg bg-kalbe-50 text-kalbe-700 border border-kalbe-200 text-xs font-bold hover:bg-kalbe-100 flex items-center space-x-1">
                ...
                <div id="passengerContainer" class="space-y-3">
                    ...
                    <div class="passenger-row p-4 rounded-2xl bg-slate-50 border border-slate-200 grid grid-cols-1 sm:grid-cols-4 gap-3 relative">
                        <div>
            ...
            <div class="pt-6 border-t border-slate-100 flex items-center justify-between">
                <a href="{{ route('admin.trips.index') }}" class="px-6 py-3 rounded-xl bg-slate-100 text-slate-700 text-xs font-bold hover:bg-slate-200">Batal</a>
                <button type="submit" class="px-8 py-3 rounded-xl kalbe-gradient text-white text-xs font-bold shadow-lg shadow-kalbe-500/20 hover:opacity-95 flex items-center space-x-2">
