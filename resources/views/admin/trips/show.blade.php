...
<div class="space-y-6 sm:space-y-8">
    ...
    <div class="p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-3xl bg-white border border-slate-200 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-4 sm:gap-6">
        <div class="space-y-2">
            <div class="flex flex-wrap items-center gap-2 sm:gap-3">
                <span class="font-mono font-extrabold text-lg sm:text-xl text-kalbe-700">{{ $trip->txtTripCode }}</span>
                @if ($trip->txtTripStatus === 'IN_PROGRESS')
                <span class="inline-flex items-center px-2.5 sm:px-3 py-1 rounded-full text-[11px] sm:text-xs font-extrabold bg-emerald-100 text-emerald-800 animate-pulse">
                    <i class="fa-solid fa-satellite-dish mr-1.5"></i> Sedang Berjalan (Live)
                </span>
                @elseif ($trip->txtTripStatus === 'SCHEDULED')
                <span class="inline-flex items-center px-2.5 sm:px-3 py-1 rounded-full text-[11px] sm:text-xs font-bold bg-blue-100 text-blue-800">
                    <i class="fa-solid fa-calendar mr-1.5"></i> Dijadwalkan
                </span>
                @elseif ($trip->txtTripStatus === 'COMPLETED')
                <span class="inline-flex items-center px-2.5 sm:px-3 py-1 rounded-full text-[11px] sm:text-xs font-bold bg-slate-100 text-slate-700">
                    <i class="fa-solid fa-check-double mr-1.5"></i> Perjalanan Selesai
                </span>
                @else
                <span class="inline-flex items-center px-2.5 sm:px-3 py-1 rounded-full text-[11px] sm:text-xs font-bold bg-red-100 text-red-700">
                    {{ $trip->txtTripStatus }}
                </span>
                @endif
            </div>
            <h2 class="text-xl sm:text-2xl font-extrabold text-slate-900">{{ $trip->txtDestination }}</h2>
            <p class="text-xs text-slate-500 max-w-2xl">{{ $trip->txtPurpose }}</p>
        </div>

        <!-- Quick Status Control -->
        <div class="flex items-center w-full lg:w-auto">
            <form action="{{ route('admin.trips.status', $trip->intDutyTrip_ID) }}" method="POST" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full lg:w-auto">
                @csrf
                <select name="txtTripStatus" class="p-2.5 rounded-xl border border-slate-200 text-xs font-bold bg-slate-50 w-full sm:w-auto">
                ...
                <button type="submit" class="px-4 py-2.5 rounded-xl bg-slate-800 text-white text-xs font-bold hover:bg-slate-900 text-center">
                    Update Status
                </button>
            </form>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5">
        <div class="p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-white border border-slate-200 shadow-sm flex items-center space-x-4">
            <div class="w-11 h-11 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-blue-100 text-blue-600 flex items-center justify-center text-lg sm:text-xl flex-shrink-0">
        ...
        <div class="p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-white border border-slate-200 shadow-sm flex items-center space-x-4">
            <div class="w-11 h-11 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-amber-100 text-amber-600 flex items-center justify-center text-lg sm:text-xl flex-shrink-0">
        ...
        <div class="p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-white border border-slate-200 shadow-sm flex items-center space-x-4">
            <div class="w-11 h-11 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-purple-100 text-purple-600 flex items-center justify-center text-lg sm:text-xl flex-shrink-0">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
        <!-- Vehicle & Driver Card -->
        <div class="p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-3xl bg-white border border-slate-200 shadow-sm space-y-4 sm:space-y-6">
            <h3 class="text-xs sm:text-sm font-extrabold text-slate-900 uppercase tracking-wider pb-3 border-b border-slate-100 flex items-center space-x-2">
            ...
            <div class="space-y-3 sm:space-y-4">
                <!-- Vehicle -->
                <div class="flex flex-col sm:flex-row sm:items-center justify-between p-3.5 sm:p-4 rounded-2xl bg-slate-50 border border-slate-200 gap-2">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-[11px] sm:text-xs font-semibold text-slate-500">{{ $trip->vehicle ? $trip->vehicle->txtPlateNumber : '-' }} • {{ $trip->vehicle ? $trip->vehicle->txtBrandModel : '-' }}</p>
                        </div>
                    </div>
                    <span class="self-start sm:self-auto px-2.5 py-1 rounded-full text-[11px] sm:text-xs font-bold bg-kalbe-50 text-kalbe-700">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between p-3.5 sm:p-4 rounded-2xl bg-slate-50 border border-slate-200 gap-2">
                    <div class="flex items-center space-x-3">
                        <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-xl kalbe-gradient text-white flex items-center justify-center font-bold text-sm flex-shrink-0">
                            <i class="fa-solid fa-user-tie"></i>
                        </div>
                        <div>
                            <h4 class="text-xs font-bold text-slate-900">{{ $trip->driver ? $trip->driver->txtDriverName : '-' }}</h4>
                            <p class="text-[11px] sm:text-xs font-medium text-slate-500">{{ $trip->driver ? $trip->driver->txtPhoneNumber : '-' }} • SIM: {{ $trip->driver ? $trip->driver->txtLicenseNumber : '-' }}</p>
                        </div>
                    </div>
                    @if ($trip->driver && $trip->driver->txtPhoneNumber)
                    <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $trip->driver->txtPhoneNumber) }}" target="_blank"
                        class="self-start sm:self-auto px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-200 font-bold text-xs hover:bg-emerald-100 flex items-center space-x-1">
        ...
        <div class="p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-3xl bg-white border border-slate-200 shadow-sm space-y-4">
            ...
            <div class="space-y-3 max-h-64 overflow-y-auto">
                ...
                <div class="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-between text-xs">
                    ...
                    @if ($p->txtPhoneNumber)
                    <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $p->txtPhoneNumber) }}" target="_blank" class="text-emerald-600 hover:text-emerald-700 font-bold p-1">
    ...
    <div class="p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-3xl bg-white border border-slate-200 shadow-sm space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between pb-3 border-b border-slate-100 gap-2">
            <div>
                <h3 class="text-sm sm:text-base font-extrabold text-slate-900">Rute Perjalanan & Rekam Jejak GPS</h3>
                <p class="text-[11px] sm:text-xs text-slate-500">Rekam jejak koordinat latitude/longitude yang dikirimkan aplikasi mobile driver.</p>
            </div>
            <span class="text-xs font-bold text-slate-600">Total: {{ $gpsTrail->count() }} Titik Telemetry</span>
        </div>

        <div id="tripDetailMap" class="w-full h-64 sm:h-80 rounded-2xl border border-slate-200 overflow-hidden"></div>
    </div>

    <!-- Bukti Dokumentasi & Foto Checkpoints Gallery -->
    <div class="p-4 sm:p-6 lg:p-8 rounded-2xl sm:rounded-3xl bg-white border border-slate-200 shadow-sm space-y-5 sm:space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-slate-100 gap-3 sm:gap-4">
            <div>
                <h3 class="text-sm sm:text-base font-extrabold text-slate-900">Bukti Dokumentasi & Laporan Foto Driver</h3>
                <p class="text-[11px] sm:text-xs text-slate-500">Bukti foto kondisi mobil sebelum jalan, struk pengisian BBM, tiba di lokasi tujuan, dan foto selesai.</p>
            </div>
            <span class="self-start sm:self-auto px-3 py-1 rounded-full text-xs font-bold bg-kalbe-50 text-kalbe-700 border border-kalbe-200">
                ...
                <div class="p-4 sm:p-5 flex-1 flex flex-col justify-between space-y-3">
                    <div class="space-y-2">
                        ...
                        <p class="text-xs font-semibold text-slate-700 flex items-start space-x-1.5">
                            <i class="fa-solid fa-map-pin text-kalbe-500 mt-0.5 flex-shrink-0"></i>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (mapContainer && trail.length > 0) {
            const detailMap = L.map('tripDetailMap', {
                zoomControl: true
            });
