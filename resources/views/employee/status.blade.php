...
<div class="py-6 sm:py-10 px-3 sm:px-6 lg:px-8 max-w-4xl mx-auto">
    <!-- Search Bar Card -->
    <div class="glass-card rounded-2xl sm:rounded-3xl p-5 sm:p-8 shadow-lg shadow-slate-200/50 mb-6 sm:mb-8">
        <div class="max-w-xl mx-auto text-center mb-5 sm:mb-6">
            <h1 class="text-xl sm:text-2xl font-extrabold text-slate-900">Lacak Status Pengajuan Dinas</h1>
        ...
        <form action="{{ route('employee.status') }}" method="GET" class="max-w-lg mx-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            ...
            <button type="submit" class="px-6 py-3 rounded-xl kalbe-gradient text-white text-xs font-bold shadow-md shadow-kalbe-500/20 hover:opacity-95 transition-all text-center">
                Cari
            </button>
        </form>
    </div>
    ...
    <div class="space-y-4">
        ...
        @if ($results->isEmpty())
        <div class="p-8 sm:p-12 text-center rounded-2xl sm:rounded-3xl bg-white border border-slate-200 shadow-sm">
            <div class="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-slate-100 text-slate-400 mx-auto flex items-center justify-center text-xl sm:text-2xl mb-3 sm:mb-4">
                <i class="fa-solid fa-file-circle-xmark"></i>
            </div>
            <h3 class="font-bold text-slate-700 text-sm sm:text-base">Tidak ada data ditemukan</h3>
            <p class="text-xs text-slate-400 mt-1">Pastikan nama atau kata kunci yang dimasukkan sudah sesuai.</p>
        </div>
        @else
        @foreach ($results as $item)
        <div class="p-4 sm:p-6 rounded-2xl bg-white border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
